"""
scrapers/jobspy.py — LinkedIn + Indeed scraper via python-jobspy.
...
from config import JOBSPY_QUERIES, JOBSPY_RESULTS_WANTED, JOBSPY_HOURS_OLD
...
# ── User-Agent pool ─────────────────────────────────────────────────────────
# Rotated per-query to reduce fingerprinting on LinkedIn/Indeed.
...
def scrape_all(queries: list[tuple] = JOBSPY_QUERIES) -> pd.DataFrame:
    """
    Run all queries and return a deduplicated, schema-normalized DataFrame.
    Each query tuple: (search_term, location, country_indeed).
    """
    ...
    # Normalise column names to shared schema (jobspy may return extra columns)
    for col in NORMALIZED_SCHEMA:
        if col not in combined.columns:
            combined[col] = None

    return combined[NORMALIZED_SCHEMA + [c for c in combined.columns if c not in NORMALIZED_SCHEMA]]
